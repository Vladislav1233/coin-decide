...
class ScreenBarCreate extends Component {
  ...
  componentDidMount() {
    const { dataCity, match } = this.props;

    const defineBar = match?.params?.id ? match?.params?.id : false;
    this.props.getRandomBar(dataCity.valueCity, defineBar);
    ...
    return (
      ...
      >
        <ScreenBar
          ...
          name={bar.name}
          prize={
            prize && Array.isArray(prize) && !!prize.length ? prize[0] : {}
          }
...
const mapStateToProps = ({ firebase, firestore, bars, users }) => {
  const targetBarId =
    firestore.ordered.bars && firestore.ordered.bars.length > 0
  ...
  return {
    ...
    prize: firestore.ordered.unique_prizes
      ? firestore.ordered.unique_prizes.filter(
          (item) => item.id === bars.typePrize[1]
        )
      : null,
...
const mapDispatchToProps = (dispatch) => {
  return {
    getRandomBar: (nameCity, defineBar) =>
      dispatch(getRandomBar(nameCity, defineBar)),
    savePromocode: (userUID, promocodeData) =>
      dispatch(savePromocode(userUID, promocodeData)),
    saveWinningPromocodeInStore: (data) =>
      dispatch(saveWinningPromocodeInStore(data)),
